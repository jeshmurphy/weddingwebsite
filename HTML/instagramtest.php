...
$tag = 'jesandjoe2017'; // tag for which ou want images
$results_array = scrape_insta_hash($tag);
//die(print_r($results_array));
...
$cnt = $results_array['entry_data']['TagPage'][0]['tag']['media']['count'];
	for ($i=0; $i < $cnt; $i++) {
		$latest_array = $results_array['entry_data']['TagPage'][0]['tag']['media']['nodes'][$i];
	 	$image_data[$latest_array['code']] = $latest_array['thumbnail_src']; // thumbnail and same sizes
	...
	echo json_encode($image_array,true);
	die();
